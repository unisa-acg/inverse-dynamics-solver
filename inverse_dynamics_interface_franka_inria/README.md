...
This dynamics interface for the real Franka Emika (FER) robot is based on a model identified by the following paper:
...
On top of these, the library [InverseDynamicsInterfaceFrankaInria](./include/inverse_dynamics_interface_franka_inria/inverse_dynamics_interface_franka_inria.hpp) has been developed, to include the following files:

* [`get_CoriolisMatrix.hpp`](./include/inverse_dynamics_interface_franka_inria/get_CoriolisMatrix.hpp): given the 7X1 vector of positions and the 7X1 vector of velocities, both in joint space, it computes the 7x7 matrix of torques related to centrifugal and Coriolis effects;
    * [`get_CoriolisMatrix_old.cpp`](./src/get_CoriolisMatrix_old.cpp): computes the Coriolis matrix by using temporary variables to store intermediate results of the computation;
        * This function is not actually needed by [the library implementation](./src/inverse_dynamics_interface_franka_inria.cpp), and is kept for legacy purposes only;
...
* [`get_FrictionTorque.hpp`](./include/inverse_dynamics_interface_franka_inria/get_FrictionTorque.hpp): given the 7X1 vector of velocities, it computes the 7x1 vector of torques due to friction;
* [`get_GravityVector.hpp`](./include/inverse_dynamics_interface_franka_inria/get_GravityVector.hpp): given the 7x1 vector of positions in joint space, it computes the 7x1 vector of torques due to gravity;
* [`get_MassMatrix.hpp`](./include/inverse_dynamics_interface_franka_inria/get_MassMatrix.hpp): given the 7x1 vector of positions in joint space, it computes the 7x7 inertia matrix.
...
```bash
colcon build --packages-up-to inverse_dynamics_interface_franka_inria
...
```bash
ros2 launch inverse_dynamics_interface_franka_inria evaluate_dynamics.launch.py
```
...
```bash
ros2 launch inverse_dynamics_interface_franka_inria evaluate_dynamics.launch.py input_bag:=<my_bag_file> output_bag:=<my_output_folder> topic:=<my_output_topic>
```
