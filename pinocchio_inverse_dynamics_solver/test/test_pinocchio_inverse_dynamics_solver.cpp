 ...
 *
 * Title:   test_pinocchio_inverse_dynamics_solver.cpp
 * Author:  Vincenzo Petrone
 * Org.:    UNISA
 * Date:    Oct 10, 2024
 *
 * This is a test for InverseDynamicsSolverPinocchio.
 *
 * -------------------------------------------------------------------
 */
...
class SharedData
{
  ...
  friend class InverseDynamicsSolverPinocchioTest;
...
class InverseDynamicsSolverPinocchioTest : public ::testing::Test
{
protected:
 ...
 */
TEST_F(InverseDynamicsSolverPinocchioTest, TestInitializationFromInputArgument)
{
 ...
 */
TEST_F(InverseDynamicsSolverPinocchioTest, TestInitializationFromNodeParametersInterface)
{
  initializeSolver();
}
 ...
 */
TEST_F(InverseDynamicsSolverPinocchioTest, TestTorques)
{
  ...
  // Compute torques
  initializeSolver();
  torques = inverse_dynamics_solver->getTorques(joint_positions, joint_velocities, joint_accelerations);
 ...
 */
TEST_F(InverseDynamicsSolverPinocchioTest, TestExternalTorques)
{
  // Joint states the dynamics will be evaluated on
  const unsigned short int N_JOINTS = 6;
  Eigen::VectorXd joint_positions(N_JOINTS);
  ...
  initializeSolver();
  torques = inverse_dynamics_solver->getTorques(joint_positions, joint_velocities, joint_accelerations);
  external_torques = inverse_dynamics_solver->getExternalTorques(joint_positions, wrench);
  total_torques = inverse_dynamics_solver->getTorques(joint_positions, joint_velocities, joint_accelerations, wrench);
 ...
 */
TEST_F(InverseDynamicsSolverPinocchioTest, TestDynamicParameters)
{
  // Number of joints
  const unsigned short int N_JOINTS = 6;
