...
def generate_test_description():
    ...
    # The node to test
    test_kdl_interface_on_3r_planar_node = Node(
        package="inverse_dynamics_interface_kdl",
        executable="kdl_interface_on_3r_planar_test",
        name="test_kdl_interface_on_3r_planar_node",
    ...
    return (
        LaunchDescription(
            [
                test_kdl_interface_on_3r_planar_node,
                KeepAliveProc(),
                ReadyToTest(),
            ]
        ),
        {"test_kdl_interface_on_3r_planar_node": test_kdl_interface_on_3r_planar_node},
    )


class TestTerminatingProcessStops(TestCase):
    def test_gtest_run_complete(self, proc_info, test_kdl_interface_on_3r_planar_node):
        proc_info.assertWaitForShutdown(
            process=test_kdl_interface_on_3r_planar_node, timeout=4000.0
        )
