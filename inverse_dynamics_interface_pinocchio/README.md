...
```bash
colcon build --packages-up-to inverse_dynamics_interface_pinocchio
...
```bash
ros2 launch inverse_dynamics_interface_pinocchio evaluate_dynamics.launch.py
```
...
```bash
ros2 launch inverse_dynamics_interface_pinocchio evaluate_dynamics.launch.py input_bag:=<my_bag_file> output_bag:=<my_output_folder> topic:=<my_output_topic>
```
...
```bash
colcon test --packages-select inverse_dynamics_interface_pinocchio
```
...
```bash
colcon test --packages-select inverse_dynamics_interface_pinocchio --event-handlers console_cohesion+
```
...
This test also assess that the torques computed with `getDynamicComponents` and `getExternalTorques` are equivalent to those computed with [`KDL`](../inverse_dynamics_interface_kdl/README.md#comparison-with-pinocchio).
...
Consequently, the [node](./test/test_inverse_dynamics_interface_pinocchio.cpp) retrieves the parameters...

```cpp
// Instantiate the node
rclcpp::NodeOptions node_options;
node_options.automatically_declare_parameters_from_overrides(true);
node = rclcpp::Node::make_shared("inverse_dynamics_interface_pinocchio_test", node_options);
