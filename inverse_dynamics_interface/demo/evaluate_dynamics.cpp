 ...
 * Date:    Feb 5, 2025
 *
 * This demo evaluates the inverse dynamics interface by reading a bag
...
public:
  ...
  EvaluateSolverNode(const std::string& node_name, const rclcpp::NodeOptions options = rclcpp::NodeOptions()) : Node(node_name, options)
  {
    ...
    // Load the inverse dynamics interface plugin
    ...
    dynamics_ = loader_->createUniqueInstance(plugin_name);
    dynamics_->initialize(this->get_node_parameters_interface(), "ids");
  ...
  void processBag() const
  {
    ...
    // Evaluate dynamics and compute torques
    for (std::size_t i = 0; i < joint_states.size(); ++i)
    {
      ...
      Eigen::VectorXd torques = dynamics_->getTorques(positions, velocities, accelerations[i]);
...
private:
  ...
  std::shared_ptr<inverse_dynamics_interface::InverseDynamicsInterface> dynamics_;
