 ...
 * Date:    May 10, 2024
 *
 * This is a test for InverseDynamicsInterfaceUR10.
 *
 * -------------------------------------------------------------------
 */
...
TEST_F(InverseDynamicsSolverUR10Test, TestDynamicParameters)
{
  ...
  using inverse_dynamics_interface_ur10::Vector6d;
  ...
  while (reader.has_next())
  {
    ...
    if (message->topic_name == topic_name)
    {
      ...
      for (std::size_t i = 0; i < n_waypoints; i++)
      {
        ...
        Vector6d torques = dynamics->getTorques(positions, velocities, accelerations);

        // Get the torques by explicit computation
        std::tuple<Matrix6d, Vector6d, Vector6d> dynamic_parameters = dynamics->getDynamicParameters(positions, velocities);
        ...
        Vector6d friction = dynamics->getFrictionVector(velocities);
